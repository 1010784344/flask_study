...
# 设置相册封面url
def set_cover_url(albums):
    for new in albums:
        cover = new.photo[randint(0, len(new.photo) - 1)].fname_t
        folder = new.user.name + '/' + new.title

        # 动态的给 album 对象添加一个 coverimgurl 属性
        coverimgurl = photoSet.url(filename=folder + '/' + cover)
        new.coverimgurl = coverimgurl
...
def album_index():
    ...
    # 推荐相册
    recmm_albums = Album.query.filter(Album.privacy != 'private',Album.recommend == 1).order_by(Album.clicknum.desc()).all()[0:4]
    set_cover_url(recmm_albums)

    # 最新相册
    new_albums = Album.query.filter(Album.privacy != 'private').order_by(Album.addtime.desc()).all()[0:4]
    set_cover_url(new_albums)

    # 最热相册
    hot_albums = Album.query.filter(Album.privacy != 'private').order_by(Album.clicknum.desc()).all()[0:4]
    set_cover_url(hot_albums)

    resp = make_response(render_template('album_index.html',recmm_albums=recmm_albums,new_albums=new_albums,hot_albums=hot_albums))
    return resp
